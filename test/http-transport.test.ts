...
// `bin/vgi-etf-first-trust-worker` and talks to it over stdin/stdout; the haybarn suite exercises
...
import { makeProductsScan, makeHoldingsScan, makeFundDetailsFunction } from "../src/functions.js";
...
test("catalog is discoverable over HTTP", async () => {
  ...
  try {
    ...
    expect(await client.catalogs()).toContain("firsttrust");
  } finally {
    rpc.close();
  }
});
...
test("products table scan round-trips over HTTP (live)", async () => {
  ...
  try {
    ...
    // Live: First Trust lists ~300 ETFs.
    ...
    // Live invariant: FTCS is in the ETF list.
    expect(rows.some((r) => r.ticker === "FTCS")).toBe(true);
  } finally {
    rpc.close();
  }
}, 30_000); // the live ETF-list fetch (~0.9 MB) can take a few seconds
